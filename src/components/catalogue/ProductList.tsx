...
export function ProductList({ products, relatedSearchTerm = '' }: { products: Product[]; relatedSearchTerm?: string }) {
  ...
  return <div className="product-list product-list--swipe" aria-label="Products. Swipe sideways to browse.">{products.map((product) => <ProductCard product={product} relatedSearchTerm={relatedSearchTerm} key={product.id} />)}</div>
}
